...
\begin{conjecture}{2}
	Let $\F = \{f_1, \dots, f_r\} \subset k[x_1, \dots, x_n]$. Then \[ \sdeg_{DRL}(\F) \leq \reg(\init_{DRL}(\F^h)). \] Note that if $(\F^h)$ is in generic coordinates, we recover Caminata and Gorla's result that \[ \sdeg_{DRL} (\F) \leq \reg(\init_{DRL} (\F^h)) = \reg(\F^h). \]
\end{conjecture}
...
\begin{center}
	\begin{tabular}{c|ccccccc}
		& $\mdeg(\F)$ & $\sdeg(\F)$ & $\sd(\F)$ & $d_\F$ & $\reg(\init(\F^h))$ & $\reg(\F^h)$ & Generic? \\
		\hline
		2.4 over $\Q$ & 5 & $\geq 20$\footnotemark{} & 11 & 11 & 20 & 15 & \xmark \\
		4.1 over $\mathbb{F}_{11}$ & 2 & 3 & 3 & 3 & 11 & 11 & \cmark \\
		4.2 over $\mathbb{F}_7$ & 6 & 6 & 6 & 3 & 8 & 8 & \xmark \\
		4.3 over $\mathbb{F}_{13}$ & 2 & 2 & 2 & 0 & 3 & 3 & \cmark \\
		4.4 over $\mathbb{F}_5$ & 4 & 16 & 9 & 9 & 17 & 13 & \xmark \\
		3.4.1 over $\mathbb{F}_7$ & 6 & 22 & 18 & 18 & 22 & 22 & \cmark
	\end{tabular}
\end{center}
...
In the table above, we see that the ideal in Example 3.4.1 of \cite{minko2021security} is in generic coordinates, but that we do not get $\sdeg(\F) = \sd(\F)$, as Conjecture 1 would have implied. For Conjecture 2, it remains to finish checking whether row reduction on $M_{\leq 20}$ yields a Gröbner basis in Example 2.4, but otherwise we see that $\sdeg_{DRL} (\F) \leq \reg(\init_{DRL}(\F^h))$. Lastly, $\sd_{DRL}(\F) \leq \reg(\F^h)$ in every case, as Conjecture 3 would suggest. \\

\noindent \textbf{Proposition.} Conjecture 2 is true.

\begin{proof}
	\color{red} To be written. \color{black}
\end{proof}
